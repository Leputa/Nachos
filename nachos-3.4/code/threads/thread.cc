...
/*******************  I hava change here **********************/
Thread::Thread(char* threadName,int p)   //优先级设置为1到32

{
	...
	thread_id=allocatedThread_Id();
	this->setPriority(p);
	priority=this->getPriority();
	if(thread_id!=-1){
		tid_pointer[thread_id]=this;
	}
}
Thread::Thread(char* threadName)   //优先级设置为1到32
...
int Thread::allocatedThread_Id(){
	for (int i=0;i<MaxThread;i++){
		if(thread_id_flag[i]==0){
			thread_id_flag[i]=1;
			return i;
		}
	}
...
void Thread::TS(){
	for (int i=0;i<MaxThread;i++){
		if(thread_id_flag[i]!=0){
			printf("thread name: %s,user_id: %d,thread_id: %d\n.",tid_pointer[i]->name,tid_pointer[i]->user_id,tid_pointer[i]->thread_id);
		}
	}
}
/***************************  end  ***************************/
